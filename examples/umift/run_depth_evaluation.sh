#!/usr/bin/env bash
# Evaluate the four frozen candidates for one completed B-continuation/D1 arm.
set -euo pipefail
...
test "$(cat "$TRAIN_LOGS/train_exit_code.txt")" = 0
test -f "$PROTOCOL"
test -f "$TOML"
test ! -e "$SELECTION"
test ! -e "$EVAL_LOGS"
...
mkdir "$SELECTION" "$EVAL_LOGS"
git rev-parse HEAD > "$EVAL_LOGS/evaluation_source_commit.txt"
printf '%s\n' "$BASE_CHECKPOINT_PATH" > "$EVAL_LOGS/base_checkpoint_path.txt"
printf '%s\n' "$CUDA_VISIBLE_DEVICES" > "$EVAL_LOGS/cuda_visible_devices.txt"
date -u +%FT%TZ > "$EVAL_LOGS/evaluation_started_at_utc.txt"
...
run_step() {
    ...
    if [[ "$rc" -ne 0 ]]; then
        printf '%s\n' "$rc" > "$EVAL_LOGS/evaluation_exit_code.txt"
        date -u +%FT%TZ > "$EVAL_LOGS/evaluation_finished_at_utc.txt"
        exit "$rc"
    fi
}
...
run_step select python -m examples.umift.depth_selection select \
    --arm "$ARM" --protocol "$PROTOCOL" --input "$SELECTION"

printf '0\n' > "$EVAL_LOGS/evaluation_exit_code.txt"
date -u +%FT%TZ > "$EVAL_LOGS/evaluation_finished_at_utc.txt"
