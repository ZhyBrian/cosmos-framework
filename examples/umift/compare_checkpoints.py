...
def compare_dataloader(a: Path, b: Path) -> dict[str, Any]:
    ...
    if set(files_a) != set(files_b):
        raise ValueError(f"dataloader rank-file mismatch: {sorted(files_a)} != {sorted(files_b)}")
    mismatches = []
    hashes = {}
    ...
    return {"rank_files": hashes, "mismatches": mismatches, "pass": not mismatches}
...
def compare_checkpoints(reference: str | Path, candidate: str | Path, *, atol: float = DEFAULT_ATOL, rtol: float = DEFAULT_RTOL) -> dict[str, Any]:
    if atol < 0 or rtol < 0:
    ...
    result["dataloader"] = compare_dataloader(root_a / "dataloader", root_b / "dataloader")
    ...
    result["pass"] = all(c["pass"] for c in result["components"].values()) and result["dataloader"]["pass"] and result["rng"]["all_sequences_identical"]
    return result
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        report = compare_checkpoints(args.reference, args.candidate, atol=args.atol, rtol=args.rtol)
